...
- **Frontend** (`frontend/src/`, `frontend/vercel.json`): Apple Light theme + dark `#050505/#ff4d67`
  diagnostic views. Zustand auth, react-hook-form, react-hot-toast, axios
  `withCredentials`. Centralized classes in `frontend/src/styles/common.js`. API client in `frontend/src/api/axios.js`, auth store in `frontend/src/store/useAuth.js`, routes in `frontend/src/components/routing/`.
- **Backend** (`Backend/`, Express ESM): `APIs/*API.js` routers + `controllers/*controller.js` handlers aggregated in `APIs/index.js` under `/api/v1`, Mongoose
  `models/*Model.js` (`strict:"throw"`), `verifyToken(...roles)` auth, `err.name` error
...
python scripts/build_drug_list.py   # OCR gazetteer from ../frontend/src/data/medicines
...
4. **Frontend → Vercel**: set project Root Directory to `frontend/` (`vercel.json` handles SPA rewrites).
   Set `VITE_API_URL=https://<backend>.onrender.com/api/v1`.
